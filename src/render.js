...
//   scripts/pi-run.sh src/render.js           # draw it on the CRT
//   node src/render.js --scene overlook --size 720x480
...
function parseArgs(argv) {
  const opts = { scene: 'overlook', png: null, size: null };
...
function usage(code, msg) {
  ...
  console.error('usage: render.js [--scene name] [--png file] [--size WxH]');
...
function main() {
  ...
  const t0 = Date.now();
  const scene = build(width, height);
